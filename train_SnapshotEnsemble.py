...
def run_valid_test_10crop(config):
    ...
    for index in range(len(augments)):
        ...
        valid_dataset = FDDataset(mode = 'test', modality=config.image_mode,image_size=config.image_size,
                                  fold_index=config.train_fold_index,augment=augment,augmentor=augments[index])
        valid_loader  = DataLoader( valid_dataset,
                                    shuffle=False,
                                    batch_size  = config.batch_size,
                                    drop_last   = False,
    ...
    submission(probs_tmp,initial_checkpoint+'_noTTA.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='r18_pretrain_48_depth_fold-1_rotate_RK0.4_black_RC0.2_SnapshotEnsemble')
    ...
    parser.add_argument('--mode', type=str, default='inferAllcycle', choices=['train','test','valid_10crop'])
    parser.add_argument('--pretrained_model', type=str, default='Cycle_9_min_acer_model.pth')#
